...
# Read in PSN, unshared, area, pop, voteA, voteB, CongRuoff, county
# TO DO: Edit column numbers appropriately based on map data.
map_data = [[eval(row[0]),eval(row[1]),eval(row[2]),eval(row[3]),eval(row[4]),eval(row[5]),eval(row[16]),row[17]] for row in reader1]
infile1.close()
